...
type LessonMode = "addition" | "simplifying" | "equivalent" | "percentage";
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {view === "progress" ? <section className="rounded-[2rem] border border-[#dce3f0] bg-white p-6 shadow-[0_18px_55px_rgba(35,57,105,.10)] sm:p-10"><p className="mb-3 text-sm font-bold uppercase tracking-[.16em] text-[#315bd6]">Progress</p><h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Recent completed lessons</h1><p className="mt-3 text-lg text-[#65708a]">Your saved progress stays in this browser.</p>{completedLessons.length === 0 ? <div className="mt-8 rounded-3xl bg-[#eef3ff] p-6 text-[#52607a]">Complete a lesson to see it here.</div> : <div className="mt-8 space-y-3">{completedLessons.map((lesson, index) => <div key={`${lesson.completedAt}-${index}`} className="flex flex-col gap-3 rounded-2xl border border-[#dce3f0] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold">{getLessonLabel(lesson.lessonType)}</p><p className="mt-1 text-sm text-[#65708a]">{new Date(lesson.completedAt).toLocaleString()}</p></div><div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm text-[#52607a] sm:grid-cols-4"><span>Accuracy: <strong className="text-[#17233f]">{lesson.accuracy}%</strong></span><span>Attempts: <strong className="text-[#17233f]">{lesson.attempts}</strong></span><span>Hints: <strong className="text-[#17233f]">{lesson.hintsUsed}</strong></span><span>Examples: <strong className="text-[#17233f]">{lesson.examplesUsed}</strong></span></div></div>)}</div>}</section> : <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
          <section className="rounded-[2rem] border border-[#dce3f0] bg-white p-6 shadow-[0_18px_55px_rgba(35,57,105,.10)] sm:p-10">
            ...
            <h1 className="text-3xl font-bold tracking-tight sm:text-4xl">{lessonMode === "addition" ? "Let's add fractions." : lessonMode === "simplifying" ? "Let's simplify fractions." : lessonMode === "equivalent" ? "Let's make equivalent fractions." : "Let's find a percent of a number."}</h1>
            <p className="mt-3 text-lg text-[#65708a]">{lessonMode === "addition" ? "We'll work through one step at a time." : lessonMode === "simplifying" ? "We'll find a common factor one step at a time." : lessonMode === "equivalent" ? "We'll multiply both parts by the same number one step at a time." : "We'll convert the percent, then find the amount one step at a time."}</p>
            <div className="my-9 rounded-3xl bg-[#eef3ff] px-8 py-9 text-center text-4xl font-bold text-[#17233f]">{lessonMode === "addition" ? <>{formatFraction(currentAdditionProblem.left)}&nbsp; + &nbsp;{formatFraction(currentAdditionProblem.right)}</> : lessonMode === "simplifying" ? formatFractionAsWritten(currentSimplificationProblem.fraction) : lessonMode === "equivalent" ? <>{formatFractionAsWritten(currentEquivalentProblem.fraction)} = ?</> : <>{currentPercentageProblem.percent}% of {currentPercentageProblem.number} = ?</>}</div>
            ...
            <div className="rounded-3xl border border-[#dce3f0] p-5">
              ...
              <div className="mt-2 flex flex-col gap-3 sm:flex-row"><input id="step" value={answer} onChange={(e) => setAnswer(e.target.value)} onKeyDown={(e) => e.key === "Enter" && checkStep()} placeholder={lessonMode === "percentage" ? "Example: 20" : "Example: numerator/denominator"} className="min-h-12 flex-1 rounded-xl border border-[#cbd5e1] px-4 text-base outline-none focus:ring-2 focus:ring-[#315bd6]"/><button onClick={isCorrect && problemIndex < activeProblemCount - 1 ? goToNextProblem : checkStep} className="min-h-12 rounded-xl bg-[#315bd6] px-5 font-bold text-white transition hover:bg-[#244bbd]">{isCorrect && problemIndex < activeProblemCount - 1 ? "Next problem" : "Check step"} <ArrowRight className="ml-1 inline" size={17}/></button></div>
            </div>
